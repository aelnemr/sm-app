                ...
                <div v-show="chatOpen && !loadingMessages">
                    <div class="row" style="max-height: 50vh; overflow: scroll; padding-bottom: 50px" ref="messageBox">
                        <div class="col-12" v-for="message in messages"
                            ...
                            <small>{{ message.sender && message.sender.name }} at {{ message.created_at }}</small>
                            <p>
                                {{ message.message }}
                            </p>
                        </div>
    ...
    methods: {
        openChat(userID) {
            ...
            if (app.chatUserID !== userID) {
                ...
                // console.log('newMessage-' + app.chatUserID + '-' + app.$root.userID);
                // Start socket.io listener
                // subscribe
                // console.log(Echo.channel('sm_app_database_newMessage-148-149')
                //     .listen('.messages.new', (data) => {
                //         console.log(data);
                //
                //         if (app.chatUserID) {
        ...
        sendMessage() {
            let app = this
            if (app.newMessage !== '') {
                axios.post('api/1.0/messages/send', {
